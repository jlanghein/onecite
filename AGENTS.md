...
```python
def classify(inspection: PageInspection, policy: Policy) -> PageKind:
...
**None at runtime.** This library decides which source supports which claim; it does not
need a model, a vector store or a network to do that. A scorer is supplied by the caller
behind a Protocol, and a deterministic lexical one ships for testing and for callers who
would rather not pay for inference.

Adding a runtime dependency needs a reason in the commit message, and a good one.

### No magic values

Every threshold, DPI, language string and limit is a named `Final` constant or a field on
a settings object the caller supplies. A bare number in a comparison is a defect, not a
style preference — nobody can tell whether `< 50` was measured or guessed.

Use an `Enum` where a value is one of a fixed set, and a dataclass where a value is a
record. A `dict[str, Any]` crossing a module boundary is a missing type.

---

## Secrets

This library holds no credentials and should never acquire any. A caller's scorer may
call a model API; that is the caller's key and the caller's problem.
